...
SRIA RT v0.3.1 - Tightened Precision + Observed-Range Validation
...
class Config:
    ...
    compact_destination_max: int = 10
    ...
    soft_duration_threshold: int = 300
    ...
    low_novelty_ratio: float = 0.50
    entropy_penalty_cap: float = 0.75

    # Candidate gates.
    gate_a_novelty_threshold: float = 0.50
    gate_a_max_duration: int = 420
    gate_a_max_destinations: int = 12
    gate_c_novelty_threshold: float = 0.50
    gate_c_min_destinations: int = 3
    gate_c_max_duration: int = 600

    # Suppression policy.
    suppress_low_novelty_without_velocity: bool = True
...
class PrecisionDetectorV031:
    ...
    def _candidate_gate(self, ep: Episode) -> str:
        ...
        # Gate A was too broad in v0.3 analysis. v0.3.1 requires source-user fanout,
        # compactness, source-user-dest novelty, novelty ratio, and size/time bounds.
        if (
            has_sud
            and has_compact
            ...
            return "A_tight_source_user_propagation"
        ...
        # Gate C: dense novel burst. This is intentionally narrow.
        if (
            has_sud
    ...
    def _finalize(self, ep: Episode) -> None:
        ...
        # v0.3.1 keeps the strict branch's key precision rule.
        ...
        if (
            ...
            ep.suppression_reason = "low_novelty_without_strong_velocity"
            self.suppressed.append(ep)
            return
...
def save_outputs(
    ...
    all_file = out_dir / "episodes_v031_all.jsonl"
    ranked_file = out_dir / "episodes_v031_ranked.jsonl"
    suppressed_file = out_dir / "episodes_v031_suppressed_entropy.jsonl"
    matches_file = out_dir / "redteam_matches_v031.jsonl"
    report_file = out_dir / "validation_report_v031.txt"
    ...
    lines: List[str] = []
    lines.append("SRIA RT v0.3.1 Tightened Precision + Observed-Range Validation")
    lines.append("=" * 80)
    lines.append("Purpose: reduce Gate A entropy while preserving blind red-team structural overlap.")
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default="v031_output")
    ...
    # Tuning knobs for v0.3.1 experiments.
    p.add_argument("--gate-a-novelty-threshold", type=float, default=0.50)
    p.add_argument("--gate-a-max-duration", type=int, default=420)
    p.add_argument("--gate-a-max-destinations", type=int, default=12)
    p.add_argument("--velocity-window", type=int, default=120)
    p.add_argument("--velocity-min-new-destinations", type=int, default=3)
    p.add_argument("--gate-b-velocity-threshold", type=float, default=0.65)
    p.add_argument("--low-novelty-ratio", type=float, default=0.50)
    p.add_argument("--no-low-novelty-suppression", action="store_true")
    ...
    print("=" * 80)
    print("SRIA RT v0.3.1 - Tightened Precision + Observed-Range Validation")
    print("=" * 80)
    ...
    config = Config(
        ...
        suppress_low_novelty_without_velocity=not args.no_low_novelty_suppression,
        ranked_limit=args.ranked_limit,
    )

    detector = PrecisionDetectorV031(config)
