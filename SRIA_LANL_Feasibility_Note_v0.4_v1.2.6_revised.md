...
## Negative-Background Evaluation, Empirical Score/Feature Geometry, Tail Stratification, Tail Triage Policy Simulation, and Bootstrap Robustness Analysis
...
**Document version:** v0.4 revised after v1.2.6  
**Date:** 2026  
...
This is a research note about a system that reconstructs behavioral episodes from enterprise authentication logs and ranks them for analyst review. The system was tested against public Los Alamos National Laboratory cybersecurity datasets. In the current phase, known redteam-associated authentication episodes were placed deep in the upper tail of a sampled background score distribution under the same learned ranker. The high-score tail was not structurally uniform: rare background top-tail episodes were mostly fanout/rescue-like, while known redteam-associated episodes were mostly compact/propagation-like. A triage simulation showed that analyst presentation order can be improved by using a balanced subtype-aware queue without retraining the model or suppressing any episode category. A follow-on bootstrap robustness analysis supported that finding at shallow review depths while keeping the conclusion bounded as small-sample evidence rather than production precision.

---

## Executive Summary

This note extends the SRIA LANL feasibility series beyond positive redteam sparse-window validation into broader negative-background pressure testing, empirical score/feature geometry analysis, tail stratification, analyst triage policy simulation, and bootstrap robustness analysis.
...
v1.2.6 then tested whether that practical result was stable under bootstrap resampling. Across 5,000 stratified bootstrap iterations, the balanced 70/20/10 policy showed positive mean improvement over score-only ranking at all tested depths, with strongest support at Top 25 and Top 50. The two-lane compact/mixed plus fanout/rescue policy was competitive at Top 100-500, while compact-only ordering failed at deeper depths. The result supports subtype-aware analyst presentation, not model retraining or autonomous detection.
...
+
a bootstrap robustness analysis layer
```
...
## 19. v1.2.6: Bootstrap and Robustness Analysis

v1.2.6 tested whether the v1.2.5 subtype-aware triage result was stable enough to treat as more than a one-off ordering artifact.
...
The observed policy results repeated v1.2.5:
...
C_balanced_70_20_10:
  ...
  Top 250:  27 / 31 = 87.10%
  Top 500:  30 / 31 = 96.77%
  Top 1000: 31 / 31 = 100.00%
```

The bootstrap improvement of the balanced policy over pure score-only ranking was:

```text
Top 25:
  mean improvement: +0.2322
  95% CI: [0.0323, 0.4194]
  P(improvement > 0): 98.70%

Top 50:
  mean improvement: +0.2785
  95% CI: [0.0323, 0.5161]
  P(improvement > 0): 98.34%

Top 100:
  mean improvement: +0.2092
  95% CI: [-0.0645, 0.4839]
  P(improvement > 0): 91.68%

Top 250:
  mean improvement: +0.1113
  95% CI: [-0.0645, 0.2903]
  P(improvement > 0): 84.24%

Top 500:
  mean improvement: +0.0408
  95% CI: [-0.0645, 0.1613]
  P(improvement > 0): 68.96%

Top 1000:
  mean improvement: +0.0323
  95% CI: [0.0000, 0.0968]
  P(improvement > 0): 64.42%
```

This supports the main v1.2.5 result, but it also makes the boundary clearer:

```text
The balanced policy is strongly supported at shallow review depths.
At deeper review depths, the improvement remains positive on average but is less decisive.
```

The compact-first policy showed why subtype-aware triage must remain balanced:

```text
B_compact_first:
  Top 25 mean improvement over score-only: +0.2407
  Top 50 mean improvement over score-only: +0.2481
  Top 250 mean improvement over score-only: -0.1269
  Top 500 mean improvement over score-only: -0.2384
  Top 1000 mean improvement over score-only: -0.2907
```

Interpretation:

```text
Compact-first ordering helps at very shallow depth but fails as a default policy because it delays mixed and fanout/rescue redteam-associated rows.
```

The two-lane policy was also competitive:

```text
E_two_lane_compact_plus_rescue:
  Top 100 mean improvement over score-only: +0.2022
  Top 250 mean improvement over score-only: +0.1295
  Top 500 mean improvement over score-only: +0.0523
```

Policy win frequency showed that E was often strongest at Top 100-500, while C dominated full recovery at Top 1000:

```text
Top 100:
  C_balanced_70_20_10: 40.29%
  E_two_lane:          42.88%

Top 250:
  C_balanced_70_20_10: 41.19%
  E_two_lane:          58.45%

Top 500:
  C_balanced_70_20_10: 37.63%
  E_two_lane:          55.00%

Top 1000:
  C_balanced_70_20_10: 76.28%
  E_two_lane:          11.86%
  A_score_only:        11.86%
```
...
```text
SRIA RT v1.2.6 supports the v1.2.5 finding that subtype-aware presentation improves early redteam-associated recovery relative to pure score-only ranking. The balanced 70/20/10 policy has the best full-recovery profile, while the two-lane policy is competitive at Top 100-500. Compact-only ordering is rejected as a default policy because it is too narrow at deeper review depths.
```

Important caveat:

```text
This is robustness evidence for analyst presentation order, not proof of production precision, calibrated compromise probability, or autonomous detection.
```

---

## 20. Current Architecture After v0.4
...
Layer 7:
  bootstrap robustness and uncertainty analysis
```
...
bootstrap uncertainty and policy-win-frequency diagnostics
```
...
| v1.2.6 supports balanced subtype-aware triage at shallow depths under bootstrap | Supported |
| Two-lane subtype presentation is competitive at Top 100-500 | Supported |
| Compact-only ordering should not be used as the default triage policy | Confirmed |
...
| Does subtype-aware triage generalize beyond the 31 accepted redteam-associated episodes? | Partly stress-tested by bootstrap; external/generalization testing still needed |
...
8. **Bootstrap confidence intervals are descriptive**  
   v1.2.6 uses 31 redteam-associated accepted episodes and 5,000 background-tail rows. The bootstrap estimates are useful for robustness pressure testing, but they remain small-sample descriptive statistics rather than deployment guarantees.

9. **Learned score is not calibrated probability**  
   A score of 0.10 or 0.30 is a ranking score, not a probability of compromise.

10. **No production false-positive rate**  
   v0.4 improves burden understanding and background pressure testing but does not establish deployment precision.

11. **Intrinsic manifold structure is not established**  
...
The best tested full-recovery policy was therefore not compact-only ordering.
...
v1.2.6 adds the uncertainty boundary:

```text
The balanced policy is strongly supported at shallow review depths.
The two-lane policy is competitive at Top 100-500.
Compact-only ordering is rejected as a default.
```

This means the system is no longer only ranking. It is beginning to expose useful structure inside the ranked tail, support analyst queue design, and quantify uncertainty around that presentation policy.

---

## 25. Immediate Next Step After v0.4

The v0.4 arc is now complete.

The next step should not be model tuning.

The next step should be background expansion and global-memory validation.

Recommended next milestone:

```text
v1.2.7 Background Expansion and Global-Memory Check
```

Purpose:

```text
Test whether the background compression, tail subtype structure, and subtype-aware triage behavior persist beyond the nine Tier B windows and under less window-local novelty inflation.
```

Recommended measurements:

```text
additional Tier B/C background windows
score floor stability across new windows
recurrence of fanout/rescue and compact/propagation tails
global pre-window memory effect on novelty-heavy background episodes
repeat v1.2.4-v1.2.6 diagnostics on expanded background samples
```

The v1.2.5 and v1.2.6 results should be treated as promising evidence for analyst presentation policy, not as final production validation.

---

## 26. Future Work

### 26.1 Background Expansion
...
### 26.2 Global-Memory Background Generation
...
### 26.4 Confidence Intervals and Bootstrap Stability
...
SRIA LANL v0.4 now adds the missing negative-background, empirical score/feature geometry, tail stratification, analyst presentation-policy, and bootstrap robustness layers.
...
10. Bootstrap robustness analysis supports the subtype-aware presentation result at shallow depths, identifies two-lane triage as competitive at Top 100-500, and rejects compact-only ordering as a default.
```

The current conclusion is restrained but stronger than before:

```text
SRIA RT has moved from accepted-episode redteam ranking into sampled negative-background pressure testing, empirical tail stratification, subtype-aware analyst triage simulation, and bootstrap robustness analysis.
```

The best current description is:

```text
SRIA is an authentication episode reconstruction and review-queue geometry system with subtype-aware analyst presentation and bootstrap-tested uncertainty boundaries.
```
...
| v1.2.6 bootstrap and robustness analysis | Complete |
| v1.2.7 background expansion and global-memory check | Proposed next |
...
**Current architecture:** SRIA episode reconstruction engine + learned ranker + explanation metadata + empirical tail stratification + subtype-aware analyst presentation + bootstrap robustness analysis  
**Next milestone:** v1.2.7 Background Expansion and Global-Memory Check  
