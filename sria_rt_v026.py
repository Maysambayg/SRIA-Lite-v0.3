#!/usr/bin/env python3
"""
sria_rt_v026.py - BLIND DETECTION (Safe Queue Eviction)
- Queue-based episode eviction (NO dropping of expiry records)
- Collapsed fanout signals
- Source-user fanout detection
- Fixed config printing
- Bounded queue via expiry processing, not truncation
"""
...
class BlindEpisodeDetector:
    ...
    def _expire_episodes(self, current_ts: int):
        """Safe queue-based expiration - NO DROPPING"""
        # Process ALL expired entries (queue is naturally ordered)
    ...
    def process(self, ts: int, source: str, dest: str, user: Optional[str]):
        ...
        # Expire old episodes (processes ALL expired, no truncation)
        ...
        if self.warmup_complete:
            ...
            # Source with many different users (detects C17693 pattern)
        ...
        if episode_key not in self.current:
            ...
            # Schedule expiration (safe - no dropping)
            expiry_time = ts + self.config.episode_window
            self.expiry_queue.append((expiry_time, episode_key))
    ...
    def finish(self) -> List[Episode]:
        # Expire all remaining episodes (process queue until empty)
        # Use a large timestamp to expire everything
        max_ts = 2**63 - 1
...
def main():
    ...
    out = Path(args.out_dir) if args.out_dir else base / "v026_output"
    ...
    config = Config(warmup_events=args.warmup)
    
    print("=" * 70)
    print("SRIA RT v0.2.6 - BLIND DETECTION (Safe Queue Eviction)")
    print("=" * 70)
    print(f"  Queue-based episode eviction (NO dropping, safe expiry)")
    print(f"  Collapsed fanout signals")
    print(f"  Source-user fanout detection")
    ...
    if redteam:
        print(f"[validation] Time range: {min(r['time'] for r in redteam)} - {max(r['time'] for r in redteam)}")
    ...
    if timestamps_seen:
        print(f"[detection] Auth timestamp range: {min(timestamps_seen)} - {max(timestamps_seen)}")
    print(f"[detection] Total auth events: {detector.total_events:,}")
