...
SRIA RT v0.3.2 - Balanced Recall + Precision Shaping
...
class Config:
    ...
    low_novelty_ratio: float = 0.35
    entropy_penalty_cap: float = 0.75

    # Candidate gates.
    gate_a_novelty_threshold: float = 0.35
    gate_a_max_duration: int = 600
    gate_a_max_destinations: int = 18
    gate_c_novelty_threshold: float = 0.40
...
class PrecisionDetectorV032:
    ...
    def _compute_entropy_penalty(self, ep: Episode) -> float:
        ...
        if ep.novelty_ratio < self.c.low_novelty_ratio:
            # v0.3.2 keeps low novelty as a shaping penalty, not a default hard survival rule.
            penalty += 0.10
    ...
    def _candidate_gate(self, ep: Episode) -> str:
        ...
        # Gate A is the balanced v0.3.2 midpoint. v0.3 analysis was too broad;
    ...
    def _finalize(self, ep: Episode) -> None:
        ...
        # v0.3.2 keeps the strict branch's key precision rule.
...
def save_outputs(
    ...
    all_file = out_dir / "episodes_v032_all.jsonl"
    ranked_file = out_dir / "episodes_v032_ranked.jsonl"
    suppressed_file = out_dir / "episodes_v032_suppressed_entropy.jsonl"
    matches_file = out_dir / "redteam_matches_v032.jsonl"
    suppressed_near_file = out_dir / "redteam_suppressed_near_matches_v032.jsonl"
    report_file = out_dir / "validation_report_v032.txt"
    ...
    lines: List[str] = []
    lines.append("SRIA RT v0.3.2 Balanced Recall + Precision Shaping")
    lines.append("=" * 80)
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default="v032_output")
    ...
    # Tuning knobs for v0.3.2 experiments.
    p.add_argument("--gate-a-novelty-threshold", type=float, default=0.35)
    p.add_argument("--gate-a-max-duration", type=int, default=600)
    p.add_argument("--gate-a-max-destinations", type=int, default=18)
    ...
    p.add_argument("--low-novelty-ratio", type=float, default=0.35)
    ...
    print("=" * 80)
    print("SRIA RT v0.3.2 - Balanced Recall + Precision Shaping")
    print("=" * 80)
    ...
    detector = PrecisionDetectorV032(config)
