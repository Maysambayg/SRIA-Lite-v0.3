...
SRIA RT v0.3.4 - Gate-Calibrated Recall Recovery
...
- Add gate-aware match counters to identify which gates actually recover red-team overlap.
...
class Config:
    ...
    # Candidate gates.
    gate_a_novelty_threshold: float = 0.30
    gate_a_max_duration: int = 720
    gate_a_max_destinations: int = 20
...
class PrecisionDetectorV034:
    ...
    def _compute_entropy_penalty(self, ep: Episode) -> float:
        ...
        if ep.novelty_ratio < self.c.low_novelty_ratio:
            # v0.3.4 keeps low novelty as a shaping penalty, not a default hard survival rule.
            penalty += 0.08
    ...
    def _candidate_gate(self, ep: Episode) -> str:
        ...
        # Gate A is the balanced v0.3.4 midpoint. v0.3 analysis was too broad;
    ...
    def _shape_episode_score(self, ep: Episode) -> None:
        ...
        # v0.3.4 recall recovery: compact non-velocity propagation should not be
    ...
    def _finalize(self, ep: Episode) -> None:
        ...
        # v0.3.4 keeps the strict branch's key precision rule.
...
def find_suppressed_near_matches(
    ...
    Purpose: identify red-team-overlapping episodes that were suppressed so v0.3.4
...
def save_outputs(
    ...
    all_file = out_dir / "episodes_v034_all.jsonl"
    ranked_file = out_dir / "episodes_v034_ranked.jsonl"
    suppressed_file = out_dir / "episodes_v034_suppressed_entropy.jsonl"
    matches_file = out_dir / "redteam_matches_v034.jsonl"
    suppressed_near_file = out_dir / "redteam_suppressed_near_matches_v034.jsonl"
    report_file = out_dir / "validation_report_v034.txt"
    ...
    lines: List[str] = []
    lines.append("SRIA RT v0.3.4 Gate-Calibrated Recall Recovery")
    lines.append("=" * 80)
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default="v034_output")
    ...
    p.add_argument("--min-score", type=float, default=0.79)

    # Tuning knobs for v0.3.4 experiments.
    p.add_argument("--gate-a-novelty-threshold", type=float, default=0.30)
    p.add_argument("--gate-a-max-duration", type=int, default=720)
    p.add_argument("--gate-a-max-destinations", type=int, default=20)
    ...
    print("=" * 80)
    print("SRIA RT v0.3.4 - Gate-Calibrated Recall Recovery")
    print("=" * 80)
    ...
    detector = PrecisionDetectorV034(config)
