...
SRIA RT v0.3.5 - Gate-Pruned A/B Calibration
...
- Prune D/E rescue gates by default after gate-aware validation showed red-team overlap came through A/B.
...
class Config:
    ...
    gate_d_novelty_threshold: float = 0.20
    gate_e_novelty_threshold: float = 0.20
    gate_e_noncompact_novelty_threshold: float = 0.35
    gate_e_max_duration: int = 720
    gate_e_max_destinations: int = 20
    gate_c_novelty_threshold: float = 0.30
    gate_c_min_destinations: int = 3
    gate_c_max_duration: int = 900

    # v0.3.5: prune rescue gates by default. v0.3.4 showed matched red-team
    # events came through Gate A and Gate B, while D/E added volume without
    # validated recovery in the capped run.
    enable_gate_d: bool = False
    enable_gate_e: bool = False
...
class PrecisionDetectorV035:
    ...
    def _compute_entropy_penalty(self, ep: Episode) -> float:
        ...
        if ep.novelty_ratio < self.c.low_novelty_ratio:
            # v0.3.5 keeps low novelty as a shaping penalty, not a default hard survival rule.
            penalty += 0.08
    ...
    def _candidate_gate(self, ep: Episode) -> str:
        ...
        # Gate A: primary validated path.
        # v0.3.4 gate-aware validation showed most accepted red-team overlap came
        # through Gate A. v0.3.5 therefore makes A the spine and returns its
        # thresholds closer to the v0.3.3 recall-recovery branch.
        if (
            has_sud
            and has_compact
        ...
        # Gate B: high velocity, but pruned.
        # v0.3.4 showed B was broad relative to its matched-redteam contribution.
        # Keep it, but require real source-user propagation or compactness and
        # cap enterprise spread.
        if (
            has_sud
            and has_velocity
            and ep.fanout_velocity_score >= self.c.gate_b_velocity_threshold
            and first_time_signal_count >= 2
            and (has_source_user_fanout or has_compact)
            and ep.duration() <= max(self.c.gate_a_max_duration, self.c.episode_window)
            and ep.destination_count() <= 30
            and ep.novelty_ratio >= 0.10
        ):
            return "B_high_velocity"

        # Gate C: dense novel burst rescue path. This stays small and interpretable.
        if (
            has_sud
        ...
        # Gate D/E are disabled by default in v0.3.5 because v0.3.4 gate-aware
        # validation showed no matched-redteam events credited to those paths.
        # They remain available for controlled ablation via CLI flags.
        if self.c.enable_gate_d:
            if (
                has_sud
                and has_compact
                and first_time_signal_count >= 2
                and ep.duration() <= self.c.gate_a_max_duration
                and ep.destination_count() <= self.c.gate_a_max_destinations
                and ep.novelty_ratio >= self.c.gate_d_novelty_threshold
            ):
                return "D_compact_novelty_rescue"

        if self.c.enable_gate_e:
            if (
                has_sud
                and has_source_user_fanout
                and first_time_signal_count >= 2
                and ep.duration() <= self.c.gate_e_max_duration
                and ep.destination_count() <= self.c.gate_e_max_destinations
                and (
                    (has_compact and ep.novelty_ratio >= self.c.gate_e_novelty_threshold)
                    or (has_velocity and ep.novelty_ratio >= self.c.gate_e_novelty_threshold)
                    or (
                        ep.destination_count() <= 12
                        and ep.duration() <= 600
                        and ep.novelty_ratio >= self.c.gate_e_noncompact_novelty_threshold
                    )
                )
            ):
    ...
    def _shape_episode_score(self, ep: Episode) -> None:
        ...
        # v0.3.5: D/E rescue paths are no longer default score boosters.
        # v0.3.4 showed they increased volume without validated recovery. Keep
        # tiny ablation-only bonuses when the corresponding gates are explicitly
        # enabled.
        if self.c.enable_gate_d:
            if (
                "first_time_source_user_to_dest" in ep.signals
        ...
        if self.c.enable_gate_e:
            if (
                ...
                and ep.novelty_ratio >= self.c.gate_e_novelty_threshold
                and ep.duration() <= self.c.gate_e_max_duration
                and ep.destination_count() <= self.c.gate_e_max_destinations
            ):
                score += 0.02
    ...
    def _finalize(self, ep: Episode) -> None:
        ...
        # v0.3.5 keeps the strict branch's key precision rule.
...
def validate(
    ...
    match_gate_counts: Counter = Counter()
    exact_start_gate_counts: Counter = Counter()
    matched_episode_gate_by_id: Dict[int, str] = {}
    ...
    for idx, rt in enumerate(redteam):
        ...
        for ep in episodes:
            ...
            if exact:
                ...
                exact_start_gate_counts[ep.candidate_gate] += 1

            match_gate_counts[ep.candidate_gate] += 1
            matched_episode_gate_by_id[ep.id] = ep.candidate_gate
    ...
    return {
        ...
        "match_candidate_gates": dict(match_gate_counts),
        "exact_start_candidate_gates": dict(exact_start_gate_counts),
        "matched_episode_candidate_gates": dict(Counter(matched_episode_gate_by_id.values())),
        "matches": matches,
    }
...
def find_suppressed_near_matches(
    ...
    Purpose: identify red-team-overlapping episodes that were suppressed so v0.3.5
...
def save_outputs(
    ...
    all_file = out_dir / "episodes_v035_all.jsonl"
    ranked_file = out_dir / "episodes_v035_ranked.jsonl"
    suppressed_file = out_dir / "episodes_v035_suppressed_entropy.jsonl"
    matches_file = out_dir / "redteam_matches_v035.jsonl"
    suppressed_near_file = out_dir / "redteam_suppressed_near_matches_v035.jsonl"
    report_file = out_dir / "validation_report_v035.txt"
    ...
    lines: List[str] = []
    lines.append("SRIA RT v0.3.5 Gate-Pruned A/B Calibration")
    lines.append("=" * 80)
    lines.append("Purpose: calibrate rescue gates after v0.3.3 widened too far while preserving observed-range recall diagnostics.")
    ...
    for gate, count in Counter(results.get("match_candidate_gates", {})).most_common():
        lines.append(f"  {gate}: {count}")
    lines.append("")
    lines.append("Matched episode candidate gates:")
    for gate, count in Counter(results.get("matched_episode_candidate_gates", {})).most_common():
        lines.append(f"  {gate}: {count}")
    lines.append("")
    lines.append("Exact-start candidate gates:")
    for gate, count in Counter(results.get("exact_start_candidate_gates", {})).most_common():
        lines.append(f"  {gate}: {count}")
    ...
    lines.append(f"  gate_d_novelty_threshold: {config.gate_d_novelty_threshold}")
    lines.append(f"  gate_e_novelty_threshold: {config.gate_e_novelty_threshold}")
    lines.append(f"  gate_e_noncompact_novelty_threshold: {config.gate_e_noncompact_novelty_threshold}")
    lines.append(f"  gate_e_max_duration: {config.gate_e_max_duration}")
    lines.append(f"  gate_e_max_destinations: {config.gate_e_max_destinations}")
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default="v035_output")
    ...
    # Tuning knobs for v0.3.5 experiments.
    ...
    print("=" * 80)
    print("SRIA RT v0.3.5 - Gate-Pruned A/B Calibration")
    print("=" * 80)
    ...
    detector = PrecisionDetectorV035(config)
    ...
    if results.get("match_candidate_gates"):
        print("  Matched redteam candidate gates:")
        for gate, count in Counter(results["match_candidate_gates"]).most_common():
            print(f"    {gate}: {count}")
